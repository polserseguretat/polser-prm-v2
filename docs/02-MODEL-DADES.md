...
| `013_push_ntfy.js` | Push ntfy: `partner_users.ntfy_topic`(hidden)/`push_enabled`/`push_subscribed_at` (+ backfill de topic), `notifications.link`, `notification_deliveries.pushed_at` (backfill per no reenviar historial) |
